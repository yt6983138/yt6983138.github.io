...
public class SaveHelper // generated by ai
{
	...
	public async Task<List<(Summary Summary, GameSave Save)>> GetGameSaves(IReadOnlyDictionary<string, float[]> difficulties)
	{
		...
		foreach (SimplifiedSave save in raw)
		{
			...
			try
			{
				rawData = await this.Client.GetByteArrayAsync(save.GameSave.Url); // note raw data is zip
			}
			...
			using (ZipFile zipFile = new(new MemoryStream(rawData)))
			{
				...
				byte[] decrypted = await this.Runtime!.InvokeAsync<byte[]>("AesDecrypt", decompressed, key, iv); // c# aes not supported (why microsoft)
			...
			// Console.WriteLine(BitConverter.ToString(summary[^lastStructSize..]));
